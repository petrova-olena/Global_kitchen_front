const Profile = () => {
  return (
    <>
      <section class="profile-container">
        {/*-- USER INFO -->*/}
        <div class="profile-card">
          <div class="profile-photo"></div>

          <div class="profile-info">
            <h2 class="profile-name">John Smith</h2>
            <p class="profile-email">john@example.com</p>
            <p class="profile-phone">+358 40 123 4567</p>

            <button class="edit-btn">Edit Profile</button>
          </div>
        </div>

        {/*-- EVENTS -->*/}
        <div class="section-block">
          <h3 class="section-title">My Events</h3>
          <ul class="list">
            <li>26.01 - 01.02 — Italian Week</li>
            <li>28.01 — Chef’s Special Day</li>
            <li>30.01 — Wine Tasting</li>
          </ul>
          <button class="action-btn">Add Event</button>
        </div>

        {/*-- RESERVATIONS -->*/}
        <div class="section-block">
          <h3 class="section-title">My Reservations</h3>
          <ul class="list">
            <li>
              Table for 2 — 28.01 — 19:00 <span class="cancel">Cancel</span>
            </li>
            <li>
              Table for 4 — 30.01 — 18:30 <span class="cancel">Cancel</span>
            </li>
          </ul>
        </div>

        {/*-- GIFT CARD -->*/}
        <div class="section-block">
          <h3 class="section-title">Gift Card</h3>
          <p>Buy a gift card for your friends or family.</p>
          <button class="action-btn">Buy Gift Card</button>
        </div>

        {/*-- RECIPE OF THE DAY -->*/}
        <div class="section-block">
          <h3 class="section-title">Recipe of the Day</h3>

          <div class="recipe-card">
            <div class="recipe-img"></div>
            <h4>Tomato Basil Pasta</h4>
            <p>Fresh tomatoes, basil, garlic and olive oil.</p>
            <button class="action-btn">Save to Notes</button>
          </div>
        </div>

        {/*-- NOTES -->*/}
        <div class="section-block">
          <h3 class="section-title">My Notes</h3>

          <textarea
            class="notes-area"
            placeholder="Write your notes here..."
          ></textarea>

          <div class="notes-actions">
            <button class="notes-btn save-btn">Save</button>
            <button class="notes-btn view-btn">View Notes</button>
          </div>
        </div>
      </section>
    </>
  );
};

export default Profile;
